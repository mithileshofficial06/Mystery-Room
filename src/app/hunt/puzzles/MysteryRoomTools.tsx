...
export const TORCH_HOME: [number, number, number] = [2.0, 2.02, -5.88];
export const BOOK_HOME: [number, number, number] = [-4.74, 1.6, -2.07];
export const FILM_HOME: [number, number, number] = [-4.76, 1.67, -2.07];
  ...
  return (
    <group position={[CLOCK_AT[0], CLOCK_AT[1], CLOCK_AT[2]]}>
      ...
      <group position={[0, 0, -0.12]}>
        ...
        {/* Just enough light inside that a dark torch in a dark hole is
            findable once the cover is off. Tiny radius, so it never leaks
            onto the wall and gives the recess away. */}
        <pointLight position={[0, 0.06, 0.02]} intensity={1.6} distance={0.75} decay={2} color="#cfd8e6" />
      </group>
        ...
        <group position={[0.38, 0, 0]}>
          ...
          <mesh rotation={[Math.PI / 2, 0, 0]} castShadow>
            ...
            <meshStandardMaterial
              color={hovered && !open ? "#6a737f" : "#4d545f"}
              roughness={0.6}
              metalness={0.25}
            />
          </mesh>
          ...
          <mesh position={[0, 0, 0.05]} rotation={[Math.PI / 2, 0, 0]}>
            ...
            <meshStandardMaterial color="#c39b52" metalness={0.65} roughness={0.4} />
          </mesh>
          <mesh position={[0, 0, 0.05]} rotation={[Math.PI / 2, 0, 0]}>
            <cylinderGeometry args={[0.29, 0.29, 0.02, 24]} />
            <meshStandardMaterial color="#efe7d5" roughness={0.9} />
          </mesh>
          {/* Hour ticks */}
